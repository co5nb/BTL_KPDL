...
public class CenterImpl implements CenterService {
    ...
    public Center predict(List<Center> centers, PointWritable pointWritable) {
        ...
        pointWritable.setDepression_severity(minMaxScaling(pointWritable.getDepression_severity(), DataUtil.DEPRESSION_SEVERITY_MIN, DataUtil.DEPRESSION_SEVERITY_MAX));
    ...
    private Float calculateDistance(Center center,PointWritable pointWritable) {
        return (float) Math.sqrt(
                        ...
                        Math.pow(center.getSex() - pointWritable.getSex(), 2) +
